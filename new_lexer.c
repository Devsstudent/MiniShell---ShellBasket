...
/*   Updated: 2022/08/01 23:43:21 by mbelrhaz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_bool	get_size_word(int *size, t_line *lst, char *line, int i)
{
	int		*cp_size;
	int		tmp;
	char	*word;
...
t_bool	handle_pipe(char *line, int *i, int *size, t_line *lst)
{
	if (line[*i - 1] && (ft_isalnum(line[*i - 1]) || line[*i - 1] == '\"'
			|| line[*i - 1] == '\''))
		if (!get_size_word(size, lst, line, *i - 1))
			return (FALSE);
	(*size) += 1;
	if (!get_size_word(size, lst, line, *i))
		return (FALSE);
	return (TRUE);
}
...
t_bool	get_size_line_words(char *line, t_line *lst)
{
	...
	if (!get_size_word(&size_word, lst, line, i - 1))
		return (FALSE);
	return (TRUE);
}
